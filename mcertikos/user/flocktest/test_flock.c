...
int flock_acquire(struct flock_t *flock, int operation)
{
    ...
    if (operation & LOCK_EX)
    {
        ...
        if (flock->state == SHARED || flock->num_sh_active > 0)
        {
            ...
            {
                printf("[flock] LOCK_EX|LOCK_NB denied: file is in shared use (non-blocking)\n");
                spinlock_release(&flock->lock);
                return EWOULDBLOCK;
            }
            printf("[flock] LOCK_EX denied: file is in shared use (added to wait list)\n");
        ...
        else
        {
            ...
            printf("[flock] LOCK_EX waiting: file is in exclusive use (added to wait list)\n");
            flock->num_ex_waiting++;
            spinlock_release(&flock->lock);
            return -3; 
        }
...
else if (operation & LOCK_SH)
{
    if (flock->state == EXCLUSIVE || flock->ex_active)
    {
        if (operation & LOCK_NB)
        {
            printf("[flock] LOCK_SH|LOCK_NB denied: file is in exclusive use (non-blocking)\n");
            spinlock_release(&flock->lock);
            return EWOULDBLOCK;
        }
        printf("[flock] LOCK_SH denied: file is in exclusive use (added to wait list)\n");
    ...
    if (flock->num_ex_waiting > 0)
    {
        if (operation & LOCK_NB)
        {
            printf("[flock] LOCK_SH|LOCK_NB denied: writers are waiting (non-blocking)\n");
            spinlock_release(&flock->lock);
            return EWOULDBLOCK;
        }
        printf("[flock] LOCK_SH waiting: writers have priority (added to wait list)\n");
        flock->num_sh_waiting++;
        printf("[flock] Wait list updated: %d shared lock requests now waiting\n", flock->num_sh_waiting);
        spinlock_release(&flock->lock);
    ...
    printf("[flock] Shared lock granted (%d active)\n", flock->num_sh_active);
    spinlock_release(&flock->lock);
    return 0;
}
...
int flock_release(struct flock_t *flock)
{
    ...
    if (flock->state == EXCLUSIVE)
    {
        ...
        if (flock->num_ex_waiting > 0)
        {
            flock->num_ex_waiting--;
            flock->ex_active = TRUE;
            printf("[flock] promoted waiting LOCK_EX\n");
            spinlock_release(&flock->lock);
            return 0;
        }
        ...
        if (flock->num_sh_waiting > 0)
        {
            ...
            flock->sh_active = TRUE;
            flock->state = SHARED;
            printf("[flock] promoted %d waiting LOCK_SH\n", flock->num_sh_active);
            spinlock_release(&flock->lock);
            return 0;
        }
    ...
    while (1)
    {
        ...
        else if (strcmp(command, "ex") == 0)
        {
            ...
            if (result == -3)
            {
                ...
                printf("Acquire LOCK_EX would block (simulated)\n");
            }
            else
            {
